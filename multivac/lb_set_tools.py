...
# changes in-place
def resolve_costume_twins(char_list):
    unique_signatures = []  
...
# changes in-place
def resolve_twins(char_list):
    tries = 0
    while(True):
        print("finding twins...")
        duplicate_ids = [i for i, char in enumerate(char_list) if char in char_list[i+1:]]
        if len(duplicate_ids) == 0: print("no twins found")
